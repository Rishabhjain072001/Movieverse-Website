...
$sql = "SELECT * FROM episodes WHERE ShowTitle = '$ShowTitle' AND ShowID = '$ShowID' ORDER BY '$EpisodeID' ";
$result = $conn->query($sql);
$arr = array();
$arr2 = array();
while($row = $result->fetch_assoc()){
		array_push($arr,$row['SourceLink']);	
...
<script>
 var list =vrr;
 var player = new Playerjs({"id":"player", file: list[EpisodeID-1], autoplay:1});
...
$sql = "SELECT * FROM Shows ORDER BY ShowID  LIMIT $ShowID, 8";
